        ...
        public void Tabell()
        {
            ...
            foreach (var z in wrs)
            {
                ...
                if (ws.Where(x => x.Day == z.Day).ToList().Count == 0)
                {
                    ...
                    var maxtemp = today.Max(m => m.Temperature);
                    weatherStats.MaxTempHour = int.Parse(today.Where(m => m.Temperature == maxtemp).Select(m => m.Hour).ToList()[0].ToString());
                    var mintemp = today.Min(m => m.Temperature);
                    weatherStats.MinTempHour = int.Parse(today.Where(m => m.Temperature == mintemp).Select(m => m.Hour).ToList()[0].ToString());
                    var avgtemp = Math.Round(today.Average(m => m.Temperature), 1);
                    weatherStats.MaxTemperature = maxtemp;
                    weatherStats.MinTemperature = mintemp;
                    weatherStats.AvgTemperature = avgtemp;
                    var maxTempOnMonth = wrs.Max(m => m.Temperature);
                    var minTempOnMonth = wrs.Min(m => m.Temperature);
                    var avgTempOnMonth = Math.Round(wrs.Average(m => m.Temperature), 1);
                    LabelMaxTemp.Text = maxTempOnMonth.ToString();
                    MaxTempDay.Text = wrs.Where(m => m.Temperature == maxTempOnMonth).Select(m => m.Day).ToList()[0].ToString();
                    MaxTempHour.Text = wrs.Where(m => m.Temperature == maxTempOnMonth).Select(m => m.Hour).ToList()[0].ToString();
                    LabelMinTemp.Text = minTempOnMonth.ToString();
                    MinTempDay.Text = wrs.Where(m => m.Temperature == minTempOnMonth).Select(m => m.Day).ToList()[0].ToString();
                    MinTempHour.Text = wrs.Where(m => m.Temperature == minTempOnMonth).Select(m => m.Hour).ToList()[0].ToString();
